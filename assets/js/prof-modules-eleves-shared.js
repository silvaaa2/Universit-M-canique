...
function normalizeDateValue(value) {
  ...
  if (typeof value?.toDate === "function") return normalizeDateValue(value.toDate().toISOString());
  if (typeof value?.seconds === "number") return normalizeDateValue(new Date(value.seconds * 1000).toISOString());
  ...
  if (guardLoader) {
    ...
    guardLoader.innerHTML = `
      <div class="prof-login-card">
        ...
        <h1>Accès prof requis</h1>
        <p class="intro">Cette catégorie est réservée aux comptes professeurs et administrateurs.</p>
...
function normalizeProgress(data = {}) {
  const checks = { ...getEmptyChecks(), ...(data.checks || {}) };
  const dates = { ...getEmptyDates(), ...(data.dates || {}) };
...
async function loadEffectifSettings() {
  ...
  if (!snap.exists()) throw new Error("Aucun réglage d'effectif n'est configuré dans Firebase.");
  ...
  if (!spreadsheetId || !gid) throw new Error("Le lien effectif ou le GID est incomplet dans le panneau admin.");
...
async function loadEffectifRows() {
  ...
  if (!response.ok) throw new Error(`Google Sheets a refusé le chargement de l'effectif (${response.status}).`);

  return normalizeEffectifRows(parseCsv(await response.text()));
}
...
async function loadStudentProgress() {
  ...
  try {
    ...
    snap.forEach(docSnap => progressById.set(docSnap.id, normalizeProgress({ firebaseId: docSnap.id, ...docSnap.data() })));
  } catch (error) {
...
function getModuleCount(key) {
  return effectifRows.reduce((count, row) => count + (getProgressForStudent(row.normalizedIdUnique).checks?.[key] === true ? 1 : 0), 0);
}
...
function renderSummary() {
  ...
  const stats = MODULE_COLUMNS.map(column => `
    <div class="modules-stat done">
      ...
      <strong>${getModuleCount(column.key)} / ${total}</strong>
    </div>
  `).join("");
  ...
  return `
    <div class="modules-cell">
      <label class="module-check ${checked ? "checked" : ""}" data-module-label>
        <input type="checkbox" data-module-check data-student-id="${escapeHtml(row.normalizedIdUnique)}" data-module-key="${escapeHtml(column.key)}" ${checked ? "checked" : ""}>
...
function renderModuleDate(row, column, progress) {
  return `
    <div class="modules-cell">
      <input class="module-date" type="date" data-module-date data-student-id="${escapeHtml(row.normalizedIdUnique)}" data-module-key="${escapeHtml(column.key)}" value="${escapeHtml(progress.dates?.[column.key] || "")}" aria-label="Date ${escapeHtml(column.label)} pour ${escapeHtml(row.studentName)}">
    </div>
  `;
}
  ...
  const body = rows.map(row => {
    ...
    return `
      <div class="modules-row" data-student-row="${escapeHtml(row.normalizedIdUnique)}">
        ...
        ${MODULE_COLUMNS.map(column => renderModuleCheck(row, column, progress) + renderModuleDate(row, column, progress)).join("")}
      </div>
    `;
  }).join("");
...
async function saveModuleCheck(input) {
  ...
  if (dateInput && nextChecked && !dateInput.value) dateInput.value = nextDate;

  const nextProgress = normalizeProgress({
    ...previousProgress,
    checks: { ...previousProgress.checks, [moduleKey]: nextChecked },
    dates: { ...previousProgress.dates, [moduleKey]: nextDate },
    completedAt: { ...(previousProgress.completedAt || {}), [moduleKey]: nextChecked ? new Date().toISOString() : null }
  });
...
async function saveModuleDate(input) {
  ...
  const nextProgress = normalizeProgress({
    ...previousProgress,
    dates: { ...previousProgress.dates, [moduleKey]: input.value || "" }
  });
...
async function loadAndRenderModules() {
  try {
    ...
    if (writesAvailable) setStatus("Données chargées.", "ok");
  } catch (error) {
...
reloadModulesBtn?.addEventListener("click", loadAndRenderModules);
...
modulesTable?.addEventListener("change", event => {
  ...
  if (dateInput) saveModuleDate(dateInput);
});
