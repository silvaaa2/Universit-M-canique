...
const { getProfAccessPolicy } = require("./prof-access-control.js");
...
function isTemporaryPolicyError(error) {
...
function getSignedPolicyFallback(claims = {}) {
  return {
    disabled: false,
    sessionVersion: String(claims.sessionVersion || "default"),
    permissions: Array.isArray(claims.sitePermissions) ? claims.sitePermissions.map(String) : []
  };
}

async function loadRequestPolicy(claims, access, policyLoader) {
  if (!policyLoader) return getSignedPolicyFallback(claims);

  try {
    return await policyLoader(claims.discordId, { admin: access.admin });
  } catch (error) {
    if (!isTemporaryPolicyError(error)) throw error;
    console.warn(
      "Contrôle d’accès Firebase temporairement indisponible, utilisation des droits signés de la session :",
      error?.message || error
    );
    return getSignedPolicyFallback(claims);
  }
}
...
async function verifyFirebaseProfAccess(idToken, options = {}) {
  ...
  const policyLoader = options.policyLoader || (options.fetchImpl ? null : getProfAccessPolicy);
  const policy = discordIdentity
    ? await loadRequestPolicy(claims, access, policyLoader)
    : getSignedPolicyFallback(claims);
  const claimedSessionVersion = String(claims.sessionVersion || "default");

  if (policy.disabled) throw authError("Ce compte Discord a été temporairement désactivé.", 403);
  if (policy.sessionVersion !== "default" && claimedSessionVersion !== policy.sessionVersion) {
    throw authError("Cette session a été déconnectée à distance.", 401);
  }
  ...
  return {
    ...
    owner: discordIdentity && claims.owner === true,
