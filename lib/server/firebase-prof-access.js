...
async function verifyFirebaseProfAccess(idToken, options = {}) {
  ...
  const policy = discordIdentity && policyLoader
    ? await policyLoader(claims.discordId, { admin: access.admin })
    : {
        disabled: false,
        sessionVersion: String(claims.sessionVersion || "default"),
        permissions: Array.isArray(claims.sitePermissions) ? claims.sitePermissions : []
      };
