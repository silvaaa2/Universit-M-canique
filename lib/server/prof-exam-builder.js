...
const AUTOMATIC_BONUS_POINTS = 2;
...
function normalizeCorrectAnswers(question, type, options) {
  if (type === "true_false") {
    ...
    const answer = cleanText(source, 12).toLowerCase();
...
function normalizeExamPayload(payload = {}, existing = null) {
  ...
  if (status === "published") {
    ...
    if (missingAnswerIndex >= 0) {
      throw new ProfAuthError(
        "answer-key",
        `Choisis au moins une bonne réponse pour la question ${missingAnswerIndex + 1}.`,
        400
      );
    }
  }
  const questionMaxScore = Math.round(questions.reduce((total, question) => total + question.points, 0) * 100) / 100;
...
function publicExam(exam) {
  const questionMaxScore = Number.isFinite(Number(exam?.questionMaxScore))
    ? Number(exam.questionMaxScore)
    : Math.round((Array.isArray(exam?.questions) ? exam.questions : []).reduce((sum, question) => sum + Number(question.points || 0), 0) * 100) / 100;
  return {
