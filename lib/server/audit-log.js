...
async function sendDiscordAudit(event, fetchImpl = fetch) {
  ...
  const fields = [
    { name: "Utilisateur", value: `${event.actorName}\n${event.actorId}`, inline: true },
    { name: "Catégorie", value: event.category, inline: true },
    { name: "Action", value: event.action, inline: false }
  ];
  if (event.target) fields.push({ name: "Cible", value: event.target, inline: false });
  if (event.details) fields.push({ name: "Détails", value: event.details, inline: false });
  ...
  const response = await fetchImpl(`${DISCORD_API_BASE}/channels/${channelId}/messages`, {
    ...
    body: JSON.stringify({
      ...
      embeds: [{
        title: event.actorType === "company" ? "Action entreprise" : "Action espace professeur",
