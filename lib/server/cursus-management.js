...
async function readEffectifSettings() {
  const stageRaw = await getDocument(SETTINGS_COLLECTION, STAGE_EFFECTIF_DOCUMENT);
  const moduleRaw = await getDocument(SETTINGS_COLLECTION, MODULE_EFFECTIF_DOCUMENT);
  return {
    stageEffectif: normalizeSheetSettings(stageRaw || {}) || null,
    moduleEffectif: normalizeSheetSettings(moduleRaw || stageRaw || {}) || null
  };
}
...
async function readCurrentData() {
  ...
  const [stages, exams, allModules] = await Promise.all([
...
async function archiveStages(access, body) {
  ...
  if (!current.stages.length && !current.exams.length && !current.modules.length) {
    throw Object.assign(new Error("Aucune donnée de cursus à archiver."), { status: 400 });
  }
  ...
  if (!existingArchive) {
    await createDocument(STAGE_ARCHIVE_COLLECTION, archiveId, {
      ...
      summary: buildStageSummary(stageRows, examRows),
...
async function archiveModules(access) {
  ...
  const students = normalizeModuleArchiveRows(activeModules);
...
async function saveEffectifSettings(access, body) {
  ...
  const documentId = target === "modules"
    ? MODULE_EFFECTIF_DOCUMENT
    : target === "stages"
      ? STAGE_EFFECTIF_DOCUMENT
      : "";
  if (!documentId) throw Object.assign(new Error("Réglage effectif inconnu."), { status: 400 });
  ...
  const now = new Date().toISOString();
  await upsertDocument(SETTINGS_COLLECTION, documentId, {
    ...settings,
