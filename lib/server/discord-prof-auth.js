...
const ACCESS_POLICY_RETRY_DELAY_MS = 180;

let googleAccessTokenCache = null;
let accessSheetCache = null;

function isTemporaryAccessPolicyError(error) {
...
async function loadProfAccessPolicyForLogin(discordId, loader, wait = ms => new Promise(resolve => setTimeout(resolve, ms))) {
  let lastError = null;

  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      return await loader(discordId, { force: attempt > 0 });
    } catch (error) {
      if (!isTemporaryAccessPolicyError(error)) throw error;
      lastError = error;
      if (attempt === 0) await wait(ACCESS_POLICY_RETRY_DELAY_MS);
    }
  }

  console.warn(
    "Permissions professeur temporairement indisponibles, accès de secours depuis la feuille autorisée :",
...
function getLoginPath() {
  // Le portail unique remplace l'ancien écran professeur, y compris si
  // DISCORD_LOGIN_PATH pointe encore vers lui sur un déploiement existant.
...
function buildDiscordAvatarUrl(user) {
  ...
  if (discriminator !== "0" && /^\d{4}$/.test(discriminator)) {
    defaultAvatarIndex = Number(discriminator) % 5;
  } else {
    try {
      defaultAvatarIndex = Number((BigInt(String(user.id)) >> 22n) % 6n);
    } catch {
...
async function authorizeDiscordLogin(code) {
  ...
  } = require("./prof-access-control.js");
  const storedPolicy = await loadProfAccessPolicyForLogin(user.id, getProfAccessPolicy);
  const ownerDiscordId = resolveOwnerDiscordId(rows);
  const owner = String(user.id) === ownerDiscordId;
  const effectiveAdmin = resolveEffectiveAdmin(access, storedPolicy, ownerDiscordId);
  const policy = normalizePolicy(storedPolicy, effectiveAdmin);
  if (policy.disabled) {
    throw new ProfAuthError("access_disabled", "Ce compte Discord a été temporairement désactivé.", 403);
  }

  const roleId = effectiveAdmin
  ...
  return {
    ...
    siteRole: effectiveAdmin ? "admin" : "prof",
...
function createFirebaseCustomToken(identity, nowSeconds = Math.floor(Date.now() / 1000)) {
  ...
  return signJwt({
    ...
    claims: {
      ...
      sitePermissions: Array.isArray(identity.permissions) ? identity.permissions : [],
      sessionVersion: String(identity.sessionVersion || "default")
    }
  });
}
